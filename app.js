...
function onsiteIcon(name, size = 16, className = "") {
  const paths = ICON_PATHS[name] || ICON_PATHS.helpCircle;
  const classAttr = className ? ` ${className}` : "";
  return `<svg class="onsite-icon${classAttr}" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="${ICON_STROKE}" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${paths}</svg>`;
}
...
function completionRingHTML(pct) {
  ...
  return `<div class="completion-mini" title="Profile ${pct}% complete">
    <svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
      <circle cx="${size / 2}" cy="${size / 2}" r="${r}" fill="none" stroke="var(--border)" stroke-width="2"/>
...
const ACTIVITY_ICONS = {
  assign: onsiteIcon("check", 14),
  score: onsiteIcon("badgeCheck", 14),
  worker: onsiteIcon("user", 14),
  job: onsiteIcon("briefcase", 14),
  avail: onsiteIcon("clock", 14),
};
...
const PROJECT_REQUIREMENT_TYPES = [
  ...
  },
  {
    ...
    shortLabel: "Video",
    description: "A video briefing workers must watch before starting.",
  },
  {
...
const PROJECT_REQUIREMENT_TIMINGS = [
  ...
  { value: "reference_anytime", label: "Reference / anytime" },
];

const PROJECT_REQUIREMENT_LEVELS = [
...
function normalizePreStartDocument(doc) {
  ...
  const hasLegacyReferenceTiming = doc?.timing === "optional_reference";
  const requirementLevel = PROJECT_REQUIREMENT_LEVELS.some(
    (item) => item.value === doc?.requirementLevel,
  )
    ? doc.requirementLevel
    : hasLegacyReferenceTiming || doc?.timing === "reference_anytime"
      ? "optional"
      : doc?.timing === "on_arrival"
        ? "required"
        : doc?.required === false
          ? "optional"
          : "required";
  let timing = hasCanonicalTiming
    ? doc.timing
    : hasLegacyReferenceTiming || doc?.required === false
      ? "reference_anytime"
      : "before_first_shift";
  ...
  return {
    ...
    required:
      requirementLevel === "required" && timing === "before_first_shift",
  };
}
...
function preStartRequirementSummary(job, workerId) {
  ...
  const required = documents.filter(
    (document) =>
      ...
      document.timing === "before_first_shift",
  );
  const onArrival = documents.filter(
    (document) =>
      document.requirementLevel === "required" &&
      document.timing === "on_arrival",
  );
...
function companyPreStartJobPanelHTML(job) {
  ...
  const rows = docs.length
    ...
    : guidedEmptyStateHTML({
        ...
        body: "This project does not have RAMS, induction or site-rule records yet. Add document records here when they are ready.",
      });
  return `
...
function clearRepeatProjectTemplate() {
  ...
  resetDraftPreStartConfiguration({ renderStep: jobWizardStep === 5 });
...
function applyRepeatProjectPreStartDraft(
  job,
  sourceLabourRequirements,
  copiedLabourRequirements,
  copiedRequirements,
) {
  const labourRequirementIds = new Map(
    sourceLabourRequirements.map((requirement, index) => [
      requirement.id,
      copiedLabourRequirements[index]?.id || "",
    ]),
  );
  draftPreStartRequirements = copiedRequirements
    .map((requirement) => {
    ...
    .filter(Boolean);
  draftPreStartSetupStatus = draftPreStartRequirements.length
    ? "configured"
    : DRAFT_PRE_START_SETUP_STATUSES.has(job.preStartSetupStatus)
...
function applyRepeatProjectToForm(job) {
  ...
  const sourceLabourRequirements = uniqueLabourRequirements(
    labourRequirementsForJob(job),
  );
  const copiedLabourRequirements = cloneRepeatLabourRequirements(job);
  const copiedPreStartRequirements = preStartDocumentsForJob(job).map(
    (doc) => ({
      ...doc,
      documentId: createId(),
      uploadedAt: new Date().toISOString(),
  ...
  pendingTradeRequirements = copiedLabourRequirements;
  applyRepeatProjectPreStartDraft(
    job,
    sourceLabourRequirements,
    copiedLabourRequirements,
    copiedPreStartRequirements,
  );
...
let draftPreStartSetupStatus = "";
let draftPreStartRequirements = [];

function resetDraftPreStartConfiguration({ renderStep = false } = {}) {
  draftPreStartSetupStatus = "";
  draftPreStartRequirements = [];
  if (renderStep) renderDraftPreStartStep();
}
...
function normalizeDraftPreStartRequirement(requirement, existing = null) {
  return normalizePreStartDocument({
    ...
    uploadedAt:
      existing?.uploadedAt || requirement?.uploadedAt || new Date().toISOString(),
    updatedAt: new Date().toISOString(),
    archivedAt: "",
  });
}

function upsertDraftPreStartRequirement(requirement) {
  const requirementId = String(requirement?.documentId || "");
  const existingIndex = requirementId
    ? draftPreStartRequirements.findIndex(
  ...
  if (existingIndex >= 0) draftPreStartRequirements[existingIndex] = normalized;
  else draftPreStartRequirements.push(normalized);
  draftPreStartSetupStatus = "configured";
  setDraftPreStartValidation();
  return { ok: true, updated: existingIndex >= 0, requirement: normalized };
}

function removeDraftPreStartRequirement(requirementId) {
  const previousLength = draftPreStartRequirements.length;
  draftPreStartRequirements = draftPreStartRequirements.filter(
    (requirement) => requirement.documentId !== requirementId,
  );
  setDraftPreStartValidation();
  return previousLength === draftPreStartRequirements.length
    ? { ok: false, reason: "Draft requirement not found." }
    : { ok: true };
}

function getDraftPreStartSubmissionPayload() {
  return {
    setupStatus: DRAFT_PRE_START_SETUP_STATUSES.has(
      draftPreStartSetupStatus,
    )
      ? draftPreStartSetupStatus
      : "",
    requirements: draftPreStartRequirements
...
function draftPreStartRequirementReadiness(requirement) {
  if (draftPreStartRequirementAssignmentIssue(requirement)) {
...
function draftPreStartRequirementRowHTML(requirement) {
  ...
  return `<article class="jw-prestart-requirement-row">
    ...
    <div class="jw-prestart-requirement-meta">
      ...
      <span>${escapeHtml(resourceSummary)}</span>
    </div>
    <span class="jw-prestart-readiness ${readiness.tone}">${escapeHtml(readiness.label)}</span>
    <div class="jw-prestart-requirement-actions">
      <button type="button" data-draft-prestart-edit="${escapeHtml(requirement.documentId)}">Edit</button>
      <button type="button" data-draft-prestart-remove="${escapeHtml(requirement.documentId)}">Remove</button>
    </div>
  </article>`;
}

function draftPreStartRequirementListHTML() {
  return PROJECT_REQUIREMENT_TIMINGS.map((timing) => {
    ...
    const label =
      timing.value === "reference_anytime"
        ? "Optional / reference"
        : timing.label;
    return `<section class="jw-prestart-requirement-group" aria-labelledby="draft-prestart-${timing.value}">
      <h4 id="draft-prestart-${timing.value}">${escapeHtml(label)}</h4>
      <div>${requirements.map(draftPreStartRequirementRowHTML).join("")}</div>
    </section>`;
  }).join("");
}
...
function draftPreStartStateHTML() {
...
function setDraftPreStartValidation(message = "") {
  const validation = document.getElementById("jobPreStartValidation");
  if (!validation) return;
  validation.textContent = message;
  validation.classList.toggle("hidden", !message);
}

function renderDraftPreStartStep() {
  document
    .querySelectorAll('input[name="jobPreStartSetupStatus"]')
    .forEach((input) => {
      input.checked = input.value === draftPreStartSetupStatus;
    });
...
function closeDraftPreStartConfirmDialog() {
  hideWithMotion(
    document.getElementById("draftPreStartConfirmModal"),
    null,
    { remove: true },
  );
}

function openDraftPreStartConfirmDialog({
  title,
  description,
  confirmLabel,
  onConfirm,
  ...
  document.getElementById("draftPreStartConfirmModal")?.remove();
  const modal = document.createElement("div");
  modal.id = "draftPreStartConfirmModal";
  ...
  const close = () => {
    closeDraftPreStartConfirmDialog();
    if (trigger instanceof HTMLElement) trigger.focus();
  };
  modal
    .querySelectorAll("[data-draft-prestart-confirm-cancel]")
    .forEach((button) => button.addEventListener("click", close));
  modal.querySelector("[data-draft-prestart-confirm]")?.addEventListener("click", () => {
    closeDraftPreStartConfirmDialog();
    onConfirm();
  });
  ...
  modal.querySelector("[data-draft-prestart-confirm-cancel]")?.focus();
}
...
document
  ...
  .forEach((input) =>
    input.addEventListener("change", (event) =>
      requestDraftPreStartSetupStatus(
...
document.addEventListener("click", (event) => {
  const add = event.target.closest("[data-draft-prestart-add]");
  if (add) {
    openDraftPreStartRequirementModal("", add);
    return;
  }
  const edit = event.target.closest("[data-draft-prestart-edit]");
  if (edit) {
    openDraftPreStartRequirementModal(edit.dataset.draftPrestartEdit, edit);
    return;
  }
  const remove = event.target.closest("[data-draft-prestart-remove]");
  if (!remove) return;
  const requirement = draftPreStartRequirements.find(
    (item) => item.documentId === remove.dataset.draftPrestartRemove,
  );
  if (!requirement) return;
  openDraftPreStartConfirmDialog({
    title: "Remove pre-start requirement?",
    description: `${requirement.documentName} will be removed from this request.`,
    confirmLabel: "Remove requirement",
    trigger: remove,
    onConfirm: () => {
      removeDraftPreStartRequirement(requirement.documentId);
      renderDraftPreStartStep();
      showToast("Pre-start requirement removed");
    },
  });
});
...
const NAV_SM = {
  home: onsiteIcon("home", 16),
  jobs: onsiteIcon("briefcase", 16),
  bookings: onsiteIcon("calendar", 16),
  profile: onsiteIcon("user", 16),
  requests: onsiteIcon("fileText", 16),
  workforce: onsiteIcon("users", 16),
  account: onsiteIcon("user", 16),
  ...
  const notificationPanel = `
    <div class="worker-home-topbar">
      <button class="worker-bell-btn" type="button" data-worker-notifications>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 7h18s-3 0-3-7"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>
  ...
  const recJobsHtml = recommended.length
    ...
    ${recommended
      ...
      <div class="wh-rec-job" data-apply-home="${job.id}">
        ...
        <div class="wh-rec-right">${urgency}<svg class="wh-rec-arrow" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg></div>
      </div>`;
      })
      .join("")}
...
function companyProjectWorkersHTML(job, summary, agreements = companyProjectAgreementsForJob(job)) {
  ...
  const approvals = summary.reviewWorkers.length
    ? `<div class="company-project-workspace-subsection">
        <div class="company-project-workspace-subhead">
          <div><span>Awaiting approval</span><strong>${summary.reviewWorkers.length} worker${summary.reviewWorkers.length === 1 ? "" : "s"}</strong></div>
        </div>
        <div class="company-project-review-list">${summary.reviewWorkers
        .map((app) => {
          const worker = applicationWorker(app);
    ...
    : "";
  return `
    <section class="company-project-workspace-card company-project-workforce-primary">
...
function companyProjectRequirementsHTML(job, summary) {
  ...
  const requirementRows = uniqueLabourRequirements(summary.labourRequirements)
    .map((req) => {
      ...
      const overtime = req.overtimeAvailable
        ? Object.entries(req.overtimeRates || {})
            .filter(([, value]) => value)
            .map(([key, value]) => `${key === "afterStandardHours" ? "After-hours" : key[0].toUpperCase() + key.slice(1)}: ${String(value).replaceAll("_", " ")}`)
            .join(" · ") || "Available"
        : "Not offered";
    ...
    .join("");
  return `
    <div class="company-project-workspace company-project-labour-workspace">
...
function companyProjectAttendanceTime(value) {
  if (!value) return "—";
...
function companyProjectAttendanceHistoryHTML(job, records) {
  if (!records.length) {
    return `<div class="company-project-workspace-empty is-compact"><strong>No attendance history yet.</strong><span>Daily records will appear after workers sign in or attendance is manually confirmed.</span></div>`;
  }
  const groups = Array.from(records.reduce((map, record) => {
    ...
    .slice(0, 12);
  return `<div class="company-project-attendance-history">
    <div class="company-project-attendance-history-head" aria-hidden="true"><span>Date</span><span>Expected</span><span>Signed in</span><span>Late</span><span>No-shows</span><span>Confirmation</span></div>
    ${groups.map(([date, dayRecords]) => {
      const signed = dayRecords.filter((record) => ["checkedIn", "onTime", "late", "sentHome"].includes(record.status) || record.checkInTime).length;
      const late = dayRecords.filter((record) => record.status === "late" || record.lateReport).length;
      const noShows = dayRecords.filter((record) => record.status === "noShow").length;
      const approvalStatuses = dayRecords.map((record) => record.commercial?.approvalStatus || record.approvalStatus || (record.supervisorConfirmed ? "manager_reviewed" : "draft"));
      const approvalStatus = approvalStatuses.every((status) => ["company_confirmed", "invoice_ready"].includes(status))
        ? approvalStatuses.includes("invoice_ready") ? "invoice_ready" : "company_confirmed"
        : approvalStatuses.includes("manager_reviewed") ? "manager_reviewed" : "draft";
      return `<div class="company-project-attendance-history-row">
        <strong>${date ? formatDateOnly(date) : "Date not set"}</strong><span>${dayRecords.length}</span><span>${signed}</span><span>${late}</span><span>${noShows}</span><span>${escapeHtml(ATTENDANCE_APPROVAL_STATUS[approvalStatus]?.label || "Draft")}</span>
      </div>`;
    }).join("")}
  </div>`;
}
...
function projectRequirementAudienceLabelFromRequirements(
  labourRequirements,
  requirement,
) {
...
function companyProjectRequirementStatusHTML(job, requirement, summary) {
  if (requirement.contentToFollow) {
    return `<span class="company-project-requirement-state is-outstanding">Content to follow</span>`;
  }
  ...
  if (requirement.requirementLevel === "optional") {
    const completion = progress.total
      ...
      : "";
    return `<span class="company-project-requirement-state is-neutral">Optional${completion}</span>`;
  }
  if (!progress.total) {
  ...
  const isBlocking =
    requirement.timing === "before_first_shift" && projectStartDays(job) <= 0;
  return `<span class="company-project-requirement-state ${isBlocking ? "is-blocking" : "is-outstanding"}">${progress.total - progress.completed} remaining</span>`;
}
...
function companyProjectRequirementsViewHTML(job, summary, requirements) {
  ...
  return PROJECT_REQUIREMENT_TIMINGS.map((timing) => {
    ...
    const groupLabel = timing.value === "reference_anytime"
      ? "Reference / optional"
      : timing.label;
    return `<section class="company-project-requirement-group" aria-labelledby="project-requirement-${escapeHtml(job.id)}-${timing.value}">
      <h3 id="project-requirement-${escapeHtml(job.id)}-${timing.value}">${escapeHtml(groupLabel)}</h3>
...
function projectWorkerRequirementReadiness(job, worker, summary, requirements) {
  ...
  const beforeStart = applicable.filter(
    (requirement) =>
      requirement.requirementLevel === "required" &&
      requirement.timing === "before_first_shift",
  );
  const onArrival = applicable.filter(
    (requirement) =>
      requirement.requirementLevel === "required" &&
      requirement.timing === "on_arrival",
  );
  const preStartComplete = beforeStart.filter((requirement) =>
    !requirement.contentToFollow &&
    workerCompletedProjectRequirement(job, worker.id, requirement),
  ).length;
  const preStartOutstanding = beforeStart.length - preStartComplete;
  const onArrivalOutstanding = onArrival.filter(
    (requirement) =>
      requirement.contentToFollow ||
      !workerCompletedProjectRequirement(job, worker.id, requirement),
  ...
  if (onArrivalOutstanding) {
    return {
      label: "On-arrival action",
      tone: "is-on-arrival",
      beforeStart,
      preStartComplete,
      onArrival,
    ...
    ${summary.assignedWorkers
      .map((worker) => {
        ...
        return `<article class="company-project-worker-completion-row">
          ...
          <span class="company-project-requirement-state ${readiness.tone}" data-label="Status">${escapeHtml(readiness.label)}</span>
        </article>`;
      })
      .join("")}
  </div>`;
...
function projectRequirementStepTwoHTML(draft) {
  const comprehension =
    ...
    draft.completionAction === "watch_comprehension"
      ? `<div class="project-requirement-comprehension" data-project-requirement-comprehension>
          <p>Comprehension check</p>
          <label class="field-label">Pass threshold
            <div class="project-requirement-percentage"><input data-project-requirement-pass type="number" min="1" max="100" required value="${draft.comprehensionCheck?.passThreshold || 80}" /><span>%</span></div>
          </label>
        </div>`
      : "";
  return `<section class="project-requirement-step" aria-labelledby="projectRequirementStepTitle">
...
function saveProjectRequirementEditor({ afterSave = null } = {}) {
  ...
  const requirementPayload = {
    ...
    comprehensionCheck: {
      ...
      questions: editor.original?.comprehensionCheck?.questions || [],
    },
  };
...
function bindProjectRequirementEditorControls(modal) {
  ...
  modal.querySelectorAll('input[name="projectRequirementLevel"]').forEach((input) => {
    input.addEventListener("change", () => {
      syncProjectRequirementEditorDraft(modal);
      const draft = projectRequirementEditorState.draft;
      if (draft.requirementLevel === "required" && draft.timing === "reference_anytime") {
        draft.timing = "before_first_shift";
      }
      renderProjectRequirementEditor({ focusHeading: false });
    });
  });
  modal.querySelectorAll('input[name="projectRequirementTiming"]').forEach((input) => {
    input.addEventListener("change", () => {
      syncProjectRequirementEditorDraft(modal);
      renderProjectRequirementEditor({ focusHeading: false });
    });
  ...
  modal.querySelector("[data-project-requirement-back]")?.addEventListener("click", () => {
    syncProjectRequirementEditorDraft(modal);
    projectRequirementEditorState.step -= 1;
    renderProjectRequirementEditor();
  });
  modal.querySelector("[data-project-requirement-form]")?.addEventListener("submit", (event) => {
    event.preventDefault();
    ...
    saveProjectRequirementEditor();
  });
  modal.querySelector("[data-project-requirement-remove]")?.addEventListener("click", (event) => {
    const editor = projectRequirementEditorState;
    const draftMode = editor?.mode === "draft";
    const requirementId = event.currentTarget.dataset.projectRequirementRemove;
    const removeRequirement = () => {
      const result = draftMode
        ? removeDraftPreStartRequirement(requirementId)
        : removePreStartDocument(editor.jobId, requirementId);
      if (!result.ok) {
        showToast(result.reason);
        return;
      }
      closeProjectRequirementModal({
        afterClose: () => {
          if (draftMode) renderDraftPreStartStep();
          else render();
        },
      });
      showToast("Pre-start requirement removed");
    ...
    if (draftMode) {
      openDraftPreStartConfirmDialog({
        title: "Remove pre-start requirement?",
        description: `${editor.draft.documentName || "This requirement"} will be removed from this request.`,
        confirmLabel: "Remove requirement",
...
function trapProjectRequirementModalFocus(modal, event) {
  if (event.key !== "Tab") return;
  const focusable = Array.from(modal.querySelectorAll('button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), summary, [tabindex]:not([tabindex="-1"])'))
...
function openProjectRequirementEditor({
  mode = "project",
  jobId = "",
  requirementId = "",
  trigger = null,
} = {}) {
  const job = mode === "draft" ? null : findJob(jobId);
  if (mode !== "draft" && !job) return;
  ...
  projectRequirementEditorState = {
    mode,
    jobId,
    requirementId: requirement?.documentId || "",
...
function openProjectRequirementModal(jobId, requirementId = "", trigger = null) {
  openProjectRequirementEditor({ mode: "project", jobId, requirementId, trigger });
}

function openDraftPreStartRequirementModal(
  requirementId = "",
  trigger = null,
) {
  openProjectRequirementEditor({
    mode: "draft",
    requirementId,
    trigger,
  });
...
function companyProjectCommercialHTML(job) {
  const invoices = (state.invoices || []).filter((invoice) =>
    invoice.projectId === job.id ||
    (invoice.lines || []).some((line) => line.jobId === job.id || line.projectId === job.id),
  );
  const requirements = uniqueLabourRequirements(labourRequirementsForJob(job));
  const rates = requirements.map((requirement) => Number(requirement.budgetMax || 0)).filter((rate) => rate > 0);
  const approvedRecords = attendanceProjectRecords(job).filter((record) =>
    ["company_confirmed", "invoice_ready"].includes(record.commercial?.approvalStatus || record.approvalStatus),
  );
  const totalInvoiced = invoices.reduce((sum, invoice) => sum + Number(invoice.totalCharge || 0), 0);
  const paidValue = invoices.filter((invoice) => invoiceEffectiveStatus(invoice) === "paid").reduce((sum, invoice) => sum + Number(invoice.totalCharge || 0), 0);
  const rateLabel = rates.length
    ? Math.min(...rates) === Math.max(...rates)
      ? `${formatMoney(rates[0])}/day`
      : `${formatMoney(Math.min(...rates))}–${formatMoney(Math.max(...rates))}/day`
    : "Not recorded";
  const invoiceRows = invoices.length
    ? [...invoices]
        .sort((a, b) => new Date(b.createdAt || b.weekEnd || 0) - new Date(a.createdAt || a.weekEnd || 0))
        .map((invoice) => {
          const status = invoiceEffectiveStatus(invoice);
          const meta = INVOICE_STATUS[status] || INVOICE_STATUS.generated;
          const projectLines = (invoice.lines || []).filter((line) => !line.jobId || line.jobId === job.id || line.projectId === job.id);
          const workerCount = new Set(projectLines.map((line) => line.workerId).filter(Boolean)).size;
          const approvedDays = projectLines.reduce((sum, line) => sum + Number(line.days || 0), 0);
          return `<div class="company-project-invoice-row">
            <div><strong>${escapeHtml(invoice.invoiceNumber || "Invoice")}</strong><span>${invoice.weekStart ? `${formatDateOnly(invoice.weekStart)}–${formatDateOnly(invoice.weekEnd || invoice.weekStart)}` : "Period not recorded"}</span></div>
            <span>${workerCount || "—"}</span><span>${approvedDays || "—"}</span><strong>${formatMoney(invoice.totalCharge || 0)}</strong><span class="company-project-commercial-status is-${escapeHtml(meta.tone || "neutral")}">${escapeHtml(meta.label)}</span>
          </div>`;
        }).join("")
    : `<div class="company-project-workspace-empty is-compact"><strong>No invoice records yet.</strong><span>Approved attendance will generate invoice records for this project.</span><button class="company-project-inline-action" type="button" data-dashboard-attendance-project="${job.id}">Open attendance &rarr;</button></div>`;
  return `<div class="company-project-workspace company-project-commercial-workspace">
    <section class="company-project-workspace-card">
      <header class="company-project-workspace-head"><div><p class="company-project-workspace-kicker">Commercial Summary</p><h2>Current project position</h2><span>Values below are taken from existing project, attendance, and invoice records.</span></div></header>
      <dl class="company-project-commercial-summary">
        <div><dt>Labour requirements</dt><dd>${requirements.length}</dd></div>
        <div><dt>Advertised worker rate</dt><dd>${escapeHtml(rateLabel)}</dd></div>
        <div><dt>Approved attendance records</dt><dd>${approvedRecords.length}</dd></div>
        <div><dt>Invoice records</dt><dd>${invoices.length}</dd></div>
        ${invoices.length ? `<div><dt>Total invoiced</dt><dd>${formatMoney(totalInvoiced)}</dd></div><div><dt>Paid value</dt><dd>${formatMoney(paidValue)}</dd></div>` : ""}
      </dl>
    </section>
    <section class="company-project-workspace-card">
      <header class="company-project-workspace-head is-compact"><div><p class="company-project-workspace-kicker">Invoices &amp; Payments</p><h2>Project invoice records</h2></div></header>
      ${invoices.length ? `<div class="company-project-invoice-table-head" aria-hidden="true"><span>Period / invoice</span><span>Workers</span><span>Approved days</span><span>Amount</span><span>Status</span></div>` : ""}
      <div class="company-project-invoice-list">${invoiceRows}</div>
    </section>
  </div>`;
}
...
function companyProjectActivityHTML(job, summary) {
  ...
  return `
    <div class="company-project-workspace company-project-activity-workspace">
      <section class="company-project-workspace-card company-project-activity-card">
        <header class="company-project-workspace-head"><div><p class="company-project-workspace-kicker">Activity</p><h2>Project activity</h2><span>A chronological record of meaningful operational changes.</span></div></header>
        ${rows || `<div class="company-project-workspace-empty is-compact"><strong>No project activity yet.</strong><span>Offer updates, attendance events, document acknowledgements and project edits will appear here as the project progresses.</span></div>`}
      </section>
    </div>`;
}
...
function projectActivityRowHTML(item) {
  ...
  const healthLevel = item.metadata?.healthLevel || item.metadata?.level || item.metadata?.to || "";
  const tone = item.type === PROJECT_ACTIVITY_TYPES.PROJECT_HEALTH_CHANGED
    ? healthLevel === "urgent" || healthLevel === "critical"
      ? "critical"
      : healthLevel === "atRisk" || healthLevel === "needsAttention"
        ? "warning"
        : healthLevel === "filled" || healthLevel === "healthy"
          ? "success"
          : "info"
    : criticalTypes.has(item.type)
      ? "critical"
      : warningTypes.has(item.type)
        ? "warning"
        : successTypes.has(item.type)
          ? "success"
          : "info";
  return `<article class="project-activity-row ${tone}">
    <time>${escapeHtml(time)}</time>
    <span class="company-action-dot" aria-hidden="true"></span>
    <div>
      <strong>${escapeHtml(item.title || "Project activity")}</strong>
      ${item.description ? `<p>${escapeHtml(item.description)}</p>` : ""}
      ${workerName ? `<small>${escapeHtml(workerName)}</small>` : ""}
    </div>
  </article>`;
}
...
function bindCompanyProjectDashboardButtons(scope) {
  ...
  scope.querySelectorAll("[data-project-qr-print]").forEach((btn) => {
    btn.addEventListener("click", () =>
      openProjectSignInPrintSheet(btn.dataset.projectQrPrint),
    );
  });
  scope.querySelectorAll("[data-project-qr-regenerate]").forEach((btn) => {
    btn.addEventListener("click", () =>
      openQrRegenerationConfirm(btn.dataset.projectQrRegenerate),
    );
  });
...
function marketOptionHTML(value, selectedValue) {
  return `<option value="${escapeHtml(value)}"${value === selectedValue ? " selected" : ""}>${escapeHtml(value)}</option>`;
}

function marketFilterSelectHTML(id, label, options, selectedValue) {
  return `<label class="field-label market-filter-field">${escapeHtml(label)}
    <select id="${escapeHtml(id)}">
      <option value="">All</option>
      ${options.map((value) => marketOptionHTML(value, selectedValue)).join("")}
    </select>
  </label>`;
}

function marketRateCopy(stats) {
  if (!stats.sampleCount) {
    return {
      title: "No comparable OnSite rate data yet",
      body: "Rates will appear once matching live labour requirements exist on the platform.",
      range: "Not enough data",
    };
  }
  if (!stats.enoughData) {
    return {
      title: "Limited OnSite sample",
      body: `${stats.sampleCount} comparable advertised rate${stats.sampleCount === 1 ? "" : "s"} found. OnSite hides medians until at least ${LABOUR_MARKET_MIN_RATE_SAMPLE} samples are available.`,
      range: "Sample too small",
    };
  }
  return {
    title: `${formatMoney(stats.median)}/day median`,
    body: `Typical OnSite platform range ${formatMoney(stats.min)}–${formatMoney(stats.max)} per day.`,
    range: `${formatMoney(stats.min)}–${formatMoney(stats.max)}`,
  };
}

function marketAvailabilityRowsHTML(model) {
  const grouped = new Map();
  model.workers.forEach((worker) => {
    const key = [
      worker.trade || "Trade unverified",
      worker.specialism || worker.grade || "Specialism unverified",
      marketWorkerLocation(worker).label,
      worker.nextAvailableDate || "Available now",
    ].join("|");
    const current = grouped.get(key) || {
      trade: worker.trade || "Trade unverified",
      specialism: worker.specialism || worker.grade || "Specialism unverified",
      location: marketWorkerLocation(worker),
      availabilityDate: worker.nextAvailableDate || "",
      count: 0,
      radiusEligible: 0,
    };
    current.count += 1;
    if (Number(worker.travelRadiusMiles || 0) > 0) current.radiusEligible += 1;
  ...
  return `<div class="market-table">
    <div class="market-table-head">
      ...
      <span>Location</span>
      <span>Travel radius eligible</span>
      <span>Availability date</span>
    </div>
    ${rows.map((row) => `<div class="market-table-row">
      <span><strong>${escapeHtml(row.trade)}</strong><small>${row.count} available</small></span>
      <span>${escapeHtml(row.specialism)}</span>
      <span>${escapeHtml(row.location.label)}${row.location.verified ? "" : " <small>estimated / unverified</small>"}</span>
      <span>${row.radiusEligible}/${row.count}</span>
      <span>${row.availabilityDate ? escapeHtml(formatDateOnly(row.availabilityDate)) : "Available now"}</span>
    </div>`).join("")}
  </div>`;
}

function marketRegionCardsHTML(model) {
  if (!model.regions.length) {
    return guidedEmptyStateHTML({
      kicker: "Regional availability",
      title: "No regional availability to show yet",
      body: "Workers with saved location data will appear here as aggregated regional OnSite platform counts.",
    });
  }
  return `<div class="market-region-grid">
    ${model.regions.slice(0, 6).map((region) => `<article class="market-region-card">
      <div>
        <span class="company-home-kicker">${region.estimated ? "ESTIMATED REGION" : "REGION"}</span>
        <h3>${escapeHtml(region.label)}</h3>
        <p>${region.count} available ${escapeHtml(pluralizeTradeLabel(region.topTrade[0] || "worker", region.count))}</p>
      </div>
      <dl>
        <div><dt>Top trade</dt><dd>${escapeHtml(region.topTrade[0])}</dd></div>
        <div><dt>Top specialism</dt><dd>${escapeHtml(region.topSpecialism[0])}</dd></div>
        <div><dt>Travel radius profiles</dt><dd>${region.radiusEligible}/${region.count}</dd></div>
      </dl>
    </article>`).join("")}
  </div>`;
}

function marketDemandSupplyHTML(model) {
  return `<section class="jw-card market-intel-card">
    <div class="company-live-site-head">
      <div>
        <p class="company-home-kicker">DEMAND &amp; SUPPLY</p>
...
function buildSiteInfoHtml(job) {
  const rows = [
    ...
    <div class="site-info-row">
      <div class="site-info-icon">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 1.18h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.96a16 16 0 0 0 6 6l.96-1.96a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7a2 2 0 0 1 1.72 2z"/></svg>
      </div>
      <div class="site-info-content">
...
const ATT_CFG = {
  ...
  sentHome: {
    icon: onsiteIcon("minusCircle", 14),
    label: "Sent Home",
    color: "var(--ink-3)",
    bg: "var(--surface-3)",
    border: "var(--border)",
  ...
  siteCancelled: {
    icon: onsiteIcon("circleSlash", 14),
    label: "Site Cancel",
    color: "#6366f1",
  ...
  reportedIssue: {
    icon: onsiteIcon("alertCircle", 14),
...
function bindCompanyAttendanceProjectControls(scope) {
  const search = scope.querySelector("#attendanceProjectSearch");
  search?.addEventListener("input", () => {
    const cursor = search.selectionStart || search.value.length;
    activeAttendanceProjectSearch = search.value || "";
    renderAttendance();
    requestAnimationFrame(() => {
      const next = document.getElementById("attendanceProjectSearch");
      if (!next) return;
      next.focus();
